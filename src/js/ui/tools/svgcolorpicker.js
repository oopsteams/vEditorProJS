...
class SvgColorpicker {
  ...
  _changeColorElement(color) {
    if (color) {
      const useList = this.colorpickerElement.querySelectorAll('use');
      useList.forEach((u) => {
        u.style.fill = color;
      });
    } else {
  ...
  _makePickerButtonElement(defaultColor) {
    // this.colorpickerElement.classList.add(`${this.cssPrefix}-button`);

    this.colorElement = this.colorpickerElement.querySelector('svg');
    // this.colorElement.className = 'color-picker-value';
    if (defaultColor) {
      this.colorElement.style.fill = defaultColor;
    } else {
      // this.colorElement.classList.add('transparent');
      this.colorElement.style.fill = 'transparent';
    }
  }
...
snippet.CustomEvents.mixin(SvgColorpicker);
