...
class AnimationTrackItem extends TrackBaseItem {
  ...
  updateText() {
    ...
    if (this.frameViews.length > 0) {
      ...
      // console.log('animationTrackItem updateText context:', this.context);
  ...
  focus() {
    ...
    // console.log('animation calcTextWidth:', this.originWidth);
  ...
  newItemFrame(callback) {
    ...
    this.newRect(width, height, options.fill);
  ...
  changeDuration({ startAt, pos, dur, callback }) {
    ...
    this.updateTimeRange(this.timeRange, callback);
  }
...
CustomEvents.mixin(AnimationTrackItem);

export default AnimationTrackItem;
