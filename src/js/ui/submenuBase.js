...
class Submenu {
  ...
  constructor(
    subMenuElement,
    { locale, name, makeSvgIcon, menuBarPosition, templateHtml, usageStatistics, cssPrefix }
  ) {
   ...
   * @private
   */
  _makeSubMenuElement({ locale, name, iconStyle, makeSvgIcon, templateHtml, cssPrefix }) {
    const iconSubMenu = document.createElement('div');
    iconSubMenu.className = `${this.cssPrefix}-menu-${name}`;
