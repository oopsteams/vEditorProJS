...
import { toInteger, assignmentForDestroy, isSupportFileApi } from '@/util';
...
class Shape extends Submenu {
  constructor(subMenuElement, { locale, makeSvgIcon, menuBarPosition, usageStatistics }) {
    ...
    this._els = {
      registerIconButton: this.selector('.tie-shape-image-file'),
   ...
   */
  addEvent(actions) {
    const registerImg = this._registerImgHandler.bind(this);
    ...
    this._els.registerIconButton.addEventListener('change', registerImg);
  ...
  _registerImgComplete(img) {
    this.options.polygonImg = img;
    ...
    this.actions.addPolygonImg();
    this.changeStandbyMode();
  }

  _registerImgHandler(event) {
    const complete = this._registerImgComplete.bind(this);
    let imgUrl;
  ...
  changeStandbyMode() {
    ...
    this._els.shapeSelectButton.classList.remove('polygon');
  }
  ...
  _changeShapeHandler(event) {
    ...
    if (button) {
      this.actions.stopDrawingMode();
      this.actions.discardSelection();
      const shapeType = this.getButtonType(button, ['circle', 'triangle', 'rect', 'polygon']);
