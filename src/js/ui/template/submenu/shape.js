...
export default ({ locale, makeSvgIcon }) => `
    <ul class="tui-image-editor-submenu-item">
        <li class="tie-shape-button">
            ...
            <div class="tui-image-editor-button polygon" style="display: none;">
                <div>
                    ${makeSvgIcon(['normal', 'active'], 'icon-polygon', true)}
                </div>
                <label> ${locale.localize('Polygon')} </label>
            </div>
        </li>
        <li class="tui-image-editor-partition">
            <div></div>
        </li>
        <li class="tie-shape-add-button">
            <div class="tui-image-editor-button" style="margin:0">
                <div>
                    <input type="file" accept="image/*" class="tie-shape-image-file">
                    ${makeSvgIcon(['normal', 'active'], 'icon-load', true)}
                </div>
                <label>
                    ${locale.localize('Custom img')}
                </label>
            </div>
        </li>
        <li class="tui-image-editor-partition">
